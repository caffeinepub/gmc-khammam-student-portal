...
function ManageAttendance() {
  ...
  const handleUpload = async (rows: any[]) => {
    const records = rows.map((r) => ({
      studentReg: String(r.registrationNumber ?? ""),
      subjectId: String(r.subjectId ?? ""),
      attendanceType:
        r.type === "practical"
          ? AttendanceType.practical
          : AttendanceType.theory,
      conductedClasses: BigInt(r.conductedClasses ?? 0),
      attendedClasses: BigInt(r.attendedClasses ?? 0),
    }));
    try {
      await bulkUpsert(records);
      toast.success(
        `${records.length} attendance record(s) uploaded successfully!`,
      );
    } catch {
      ...
      <Tabs defaultValue="individual">
        ...
        <TabsContent value="bulk">
          ...
          <ExcelUploadCard
            ...
            instructions="type should be 'theory' or 'practical'. subjectId must match a subject ID."
            columns={[
              "registrationNumber",
              "subjectId",
              "type",
    ...
    const records = rows.map((r) => ({
      ...
      marksType:
        r.type === "practical"
          ? AttendanceType.practical
          : AttendanceType.theory,
      marks: BigInt(r.marks ?? 0),
    }));
    try {
      await bulkUpsert(records);
      toast.success(`${records.length} marks record(s) uploaded!`);
    } catch {
      ...
      <Tabs defaultValue="individual">
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setMarksForm((f) => ({ ...f, marks: e.target.value }))
                    }
                  />
                </div>
                <div className="md:col-span-2">
        ...
        <TabsContent value="bulk">
          <ExcelUploadCard
            ...
            instructions="marks should be a number 0-100. type: 'theory' or 'practical'."
            columns={["registrationNumber", "subjectId", "type", "marks"]}
            onUpload={handleUpload}
            loading={isPending}
          />
        </TabsContent>
...
function StudentDetailEditor({ reg }: { reg: string }) {
  ...
  if (!initialized && dashboard) {
    const initial = dashboard.subjects.map((s) => ({
      ...
      theoryMarks: String(s.theoryMarks?.marks ?? 0),
      practMarks: String(s.practicalMarks?.marks ?? 0),
    }));
    setRows(initial);
    setInitialized(true);
  }
      ...
      const marksRecords = rows.flatMap((row) => [
        {
          ...
          marks: BigInt(Number(row.theoryMarks) || 0),
        },
        {
          studentReg: reg,
          subjectId: row.subjectId,
          marksType: AttendanceType.practical,
          marks: BigInt(Number(row.practMarks) || 0),
        },
      ]);
      await Promise.all([
